    ...
    while (running) {
        ...
        (void)ticked;   /* every frame is drawn; the tick only advances state */
        {
            if (map_view) {
                ...
                } else if (ai && wai_player_look(ai) >= 0 && cars_raw) {
                    /* head-turn toward the incoming opponent: the position
                     * walker's CARS head-turn frames (mild then HARD held,
                     * then back); takes priority over the steering lean */
                    kf = cars_raw + ((size_t)kart_id * 12 +
